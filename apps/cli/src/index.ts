...
export { DEFAULT_DATABASE, parseCommandLine, renderHelp, run, type ChatHost, type ParsedCommandLine } from './cli.js';
export {
  CHAT_BANNER,
  describeSubject,
  paint,
...
export { PROVIDER_NAMES, isProviderName, providerFor, resolveModel, type ProviderName } from './providers.js';
...
// `chat` contains zero AI logic: it reads a line, hands it to RepositoryAnswerer, prints what streams back
// and formats the result. The model arrives already resolved, and the only access to the repository is a
// ContextSource — one method. providers.ts is the single place a vendor is named.
//
