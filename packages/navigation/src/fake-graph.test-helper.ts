 ...
 *
 * The navigation suite runs against this rather than a database: if the layer works with no SQLite
 ...
 * fail against SQLite. It counts calls, which is what lets one-graph-read reuse be asserted.
 */
...
export function unresolved(input: {
  ...
  return {
    ...
    sourceId: input.sourceId as NodeId,
    name: null,
    reason: input.reason ?? 'handler-not-linked',
    text: input.text,
    provenance: {
      producer: 'framework',
      fileId: 'file:src/a.ts' as NodeId,
